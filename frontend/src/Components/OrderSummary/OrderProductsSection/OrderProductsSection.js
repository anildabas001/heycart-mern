...
import classes from './OrderProductsSection.module.css';
...
const OrderProductsSection = (props) => {     
    ...
    {   props.order.shippingAddress ?
        <div className={classes.OrderProductsSection}>
            ...
            <div style={{marginTop: '20px'}} className={classes.ItemsSection}>
                <h2>Grocery Items</h2>
                <p className={classes.Error}>{props.order.error}</p>
                <div>
                    <table style={{marginBottom: '20px'}} className={classes["responsive-table"]}>
                        <thead>
                          <tr>
                            <th>Item</th>
                            <th>Quantity</th>
    ...
    : <Redirect to="/checkout/shippingaddress"/> }</>
    );
}

export default withRouter(OrderProductsSection);